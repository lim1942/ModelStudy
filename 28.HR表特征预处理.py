...
def hr_modeling(features,label):
    ...
    print("验证集 -准确率ACC :",accuracy_score(Y_validation,Y_pred))
    print("验证集 -召回率REC :",recall_score(Y_validation,Y_pred))
    print("验证集 -F-Score :",f1_score(Y_validation,Y_pred))
    print("验证集 -准确率ACC_n5 :",accuracy_score(Y_validation,Y_pred_n5))
    print("验证集 -召回率REC_n5 :",recall_score(Y_validation,Y_pred_n5))
    print("验证集 -F-Score_n5 :",f1_score(Y_validation,Y_pred_n5))
    print()

    # 针对训练集
    Y_pred = knn_clf.predict(X_train)
    print("训练集 -准确率ACC :",accuracy_score(Y_train,Y_pred))
    print("训练集 -召回率REC :",recall_score(Y_train,Y_pred))
    print("训练集 -F-Score :",f1_score(Y_train,Y_pred))
    print()

    # 针对测试集（小于训练集，表示存在微小的过拟合）
    Y_pred = knn_clf.predict(X_test)
    print("测试集 -准确率ACC :",accuracy_score(Y_test,Y_pred))
    print("测试集 -召回率REC :",recall_score(Y_test,Y_pred))
    print("测试集 -F-Score :",f1_score(Y_test,Y_pred))
